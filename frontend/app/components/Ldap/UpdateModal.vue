...
async function updateLdap(){
  ...
  const res = await $fetch<{ message: string }>('/server/ldap', { method: 'put', body: body.data })
...
<template>
  <UModal v-model:open="model" title="Update Ldap Connection" :ui="{ footer: 'justify-end' }">
    <template #body>
      <UForm :schema="ldapSchema" :state class="w-full space-y-4 md:space-y-6" @submit="updateLdap">
